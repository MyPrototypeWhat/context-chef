...
**The problem:** `@context-chef/ai-sdk-middleware` constructs a fresh `Offloader` inside `truncateToolResults()` on every request. That instance's in-memory index is single-use and discarded — calling `cleanup()` on it would do nothing useful.
