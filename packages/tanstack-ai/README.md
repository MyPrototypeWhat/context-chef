...
| `clear` | `ClearTarget[]` | No | Placeholder-style tool-result/thinking clearing. Cleared tool results become `'[Old tool result content cleared]'` and thinking is nulled — message structure stays intact, unlike `compact` which deletes. Runs AFTER compression. When tool results are targeted, an explainer instruction is auto-appended to `systemPrompts`. `ClearTarget` is exported from `@context-chef/core`. |
